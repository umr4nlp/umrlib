...
# modified by jchun
os.environ['TOKENIZERS_PARALLELISM'] = 'true'  # silence warning during smatch
...
def compute_scores(test_fn, gold_fn):
  ...
  out = precision, recall, f_score = compute_smatch(test_entries, gold_entries)
...
def output_score(stype, precision, recall, f_score):
  ...
  logger.info('%-16s -> P: %.3f,  R: %.3f,  F: %.3f' % (stype, precision, recall, f_score))
